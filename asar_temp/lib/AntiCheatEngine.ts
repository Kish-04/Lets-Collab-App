...
export class AntiCheatEngine {
  ...
  async calibrate(videoElement: HTMLVideoElement, audioStream?: MediaStream): Promise<void> {
    ...
    this.setStatus("loading", "Calibrating AI thresholds... (10s)");
    console.log("[AntiCheat] Starting 10-second ambient calibration phase...");
    ...
    const TOTAL_SAMPLES = 100; // Sample every 100ms for 10s
      ...
      const sampleInterval = setInterval(() => {
        ...
        if (samplesCollected >= TOTAL_SAMPLES) {
          ...
          if (volumeSamples.length > 0) {
            ...
            // set threshold 15 "dB" units above ambient noise floor
            newAudioThreshold = avgNoise + 15;
            if (newAudioThreshold < 10) newAudioThreshold = 25; // minimum threshold
          }
