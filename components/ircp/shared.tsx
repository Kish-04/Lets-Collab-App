...
// Activity Heatmap Component (GitHub-style calendar)
export function ActivityHeatmap({ 
  ...
  // Generate dummy data if none provided (last 12 weeks)
  const heatmapData = data || Array.from({ length: 84 }).map((_, i) => ({
    date: new Date(Date.now() - (83 - i) * 86400000).toISOString().split('T')[0],
    count: Math.random() > 0.5 ? Math.floor(Math.random() * 15) : 0
  }))
  ...
  return (
    <div className={cn("inline-flex gap-1.5 p-4 bg-[var(--surface)]/50 border border-[var(--border)]/60 rounded-xl overflow-x-auto custom-scrollbar items-end", className)}>
      {weeks.map((week, wIdx) => (
        <div key={wIdx} className="flex flex-col gap-1.5">
          {week.map((day, dIdx) => (
            <div 
              key={dIdx} 
              className={cn("w-3 h-3 rounded-sm border transition-all duration-300 hover:scale-150 cursor-crosshair group relative", getColor(day.count))}
            >
              <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 px-2 py-1 bg-[var(--elevated)] border border-[var(--border-bright)] rounded-md text-[10px] font-mono text-[var(--text-primary)] opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50">
                {day.count} events on {day.date}
              </div>
            </div>
          ))}
        </div>
