...
async function verifyRoomRouting() {
  ...
  client1.on('connect', () => {
    setTimeout(() => {
      ...
      passed++;
      // FIXED: pass arguments sequentially
      client1.emit('join-room', mockRoom, 'controller', { name: "Audit Bot 1", sessionMode: "supervised" });
    }, 500);
  });
  ...
  client1.on('join-accepted', () => {
    ...
    client2.on('connect', () => {
      setTimeout(() => {
        ...
        passed++;
        client2.emit('join-room', mockRoom, 'controller', { name: "Audit Bot 2", sessionMode: "collaborative" });
      }, 500);
    });
